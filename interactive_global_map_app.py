...
@st.cache_data
def apply_jitter(df, scale=0.001):
    random.seed(42)  # Fix randomness for consistency
    df['lat_jittered'] = df['primary_site_lat'].apply(lambda val: val + random.uniform(-scale, scale))
    df['long_jittered'] = df['primary_site_long'].apply(lambda val: val + random.uniform(-scale, scale))
    return df
...
try:
    final_df = load_data()
    final_df = apply_jitter(final_df)
